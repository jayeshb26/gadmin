                ...
                <div class="view view-cascade gradient-card-header blue-gradient narrower py-2 mx-4 mb-3 text-center justify-content-between align-items-center">
                    <div class="row">
                                                ...
                                                <tr>
                                                    <td class="text-left">
                                                        <table class="table table-bordered" id="innerTable">
                                                            <tbody>
                                                                @for ($i = 1; $i <= 4; $i++) <tr>
                                                                    @for ($j = 1; $j <= 10 && $no < 37; $j++) @php $bull=false; $length=2; @endphp @if (in_array($no, $red)) @if ($no===0) <td class="No text-center" id="{{ $no }}">
                                                                        <table>
                                                                            <tr>
                                                                                <td style="text-align: center">
                                                                                    <span style="font-size: 20px; padding-bottom: 1rem;">{{ $no }}</span>
                                                                                </td>
                                                                            </tr>
                                                                            <tr>
                                                                                <td>
                                                                                    <span style="font-size: 15px; border: 1px solid #000000; padding: 1px 5px; color: #000000; background-color: #3cff00;" id='spot{{ $no }}'>0</span>
                                                                                </td>
                                                                            </tr>
                                                                        </table>
                                                    </td>
                                                    @else
                                                    <td class="No text-center" id="{{ $no }}">
                                                        <table>
                                                            <tr>
                                                                <td style="text-align: center">
                                                                    <span style="font-size: 20px; padding-bottom: 1rem;">{{ $no }}</span>
                                                            ...
                                                            <tr>
                                                                <td>
                                                                    <span style="font-size: 15px; border: 1px solid #050505; padding: 1px 5px; color: rgb(255, 255, 255); background-color: #ff0000;" id='spot{{ $no }}'>0</span>
                                                                </td>
                                                            </tr>
                                                        </table>
                                                    </td>
                                                    ...
                                                    <td class="No text-center" id="{{ $no }}">
                                                        <table>
                                                            ...
                                                            <tr>
                                                                <td>
                                                                    <span style="font-size: 15px; border: 1px solid #000000; padding: 1px 5px; color: rgb(255, 255, 255); background-color: #000000;" id='spot{{ $no }}'>0</span>
                                                                </td>
                                                            </tr>
                                                        </table>
                                                    </td>
                        ...
                        <div class="col-sm-3 mt-2">
                            ...
                            <form action=""></form>
                            <p>
                                <select name="boosterId" id="boosterId" class="browser-default custom-select" style="width:100%">
                                    @for ($i = 1; $i <= 20; $i++) @if ($i==1) <option value="0" selected>1</option>
                                        @else
                            ...
                            <div class="mt-2 mb-3 d-flex">
                                <input type="number" class="form-control mr-2" name="SelectedCard" id="SelectedCard" style="width:100px" />
                                <input type="hidden" class="form-control mr-2" name="SelectedCardNumber" id="SelectedCardNumber" style="width:100px" readonly />
                            ...
                            <div class="table-responsive">
                                <table class="tlb table-bordered table-sm" id="resTab">
                                    <tr id="lastbet">
                                        @for ($i = 5; $i < 10; $i++) <td class="r_color_2 text-xl" id="r{{ $i }}"> Your Content Here
                                            </td>
                                            @endfor
                                    </tr>
                                </table>
...
if (card >= 0 && card <= 36 || card==00) { console.log($('#SelectedCard').val()); console.log($('#boosterId').val()); $.ajax({ type: "POST" , url: "lucky16config" , data: { card: $('#SelectedCard').val(), boosterId: $('#boosterId').val(), gametype: 'roulette_zero_3d' , _token: $('input[name="_token" ]').val() }, success: function(result) { $('#SelectedCard').val(''); $('#SelectedCardNumber').val(''); $('#alertId').addClass('show'); $('#alertId').html("Success"); removeAlert(); $('#setNo').val(''); $('input[type="radio" ]').prop("checked", false); $('#boosterId').val(0); }, error: function(result) { $('#alertIdR').addClass('show'); $('#alertIdR').html(result.responseJSON.errors.card[0]); removeAlert(); } }); } }); $('#reset').on('click', function() { $.ajax({ type: "POST" , url: "game_configs" , data: { gamename: "playSmart" , _token: $('input[name="_token" ]').val() }, success: function(result) { // console.log("vijay"); window.location.reload(); }, error: function(result) { console.log(result); } }); }); </script> --}}
